...
def main():
    ...
    parser.add_argument(
        "--rules", 
        default="rules.json", 
        help="Rules file path (default: rules.json)"
    )
    parser.add_argument(
        "--dry-run", 
        action="store_true", 
        ...
        with database.get_session() as session:
            ...
            for email in emails:
                ...
                if actions:
                    ...
                    if args.dry_run:
                        logger.info(f"[DRY RUN] Email {email.id} would execute {len(actions)} actions")
            ...
            if args.dry_run:
                logger.info(f"[DRY RUN] Would process {processed_count} emails with {action_count} total actions")
            else:
                logger.info(f"Processed {processed_count} emails with {action_count} total actions")
